...
(define (glossary-handler doc)
  ...
  (when (pair? glossary-entries)
    (define base (regexp-replace "\\.html$" (regexp-replace ".*/" here-path-from-project-root "") ""))
    (define dir  (regexp-replace "[^/]*$" here-path-from-project-root ""))
    (define sidecar-path
      (build-path *project-root* (string-append dir "._glossary_" base ".rkt")))
    (call-with-output-file sidecar-path
      (lambda (o)
